...
- Task and Console journals use colored event markers and labeled metadata badges:
  ...
  Dates use Today/Yesterday or a short local date (year only when different),
  stacked above a simple time in the phone's 12/24-hour format. Seconds and timezone
  suffixes are omitted. Labels refresh while the screen is active and on return;
  local calendar days handle midnight and DST. Invalid timestamps stay visible.
